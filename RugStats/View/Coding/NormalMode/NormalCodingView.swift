        ...
        VStack {
            ForEach(actionList, id:\.self) { actionName in
                HStack(alignment: .center){
                    NormalActionButton(timeline: $gameInfo.timeline, teamName: gameInfo.team1.teamName, actionName: actionName, gameClock: gameClock).tint(.pink)
                    NormalActionButton(timeline: $gameInfo.timeline, teamName: gameInfo.team2.teamName, actionName: actionName, gameClock: gameClock).tint(.indigo)
                }
            }
